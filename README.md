...
In one terminal, run `bundle` to install the dependencies. Run `rake db:setup` to create the databases (called rails_project_development by default). Run `rails s` to run the server.
